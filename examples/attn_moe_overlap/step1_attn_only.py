"""Step 1: Attention-only training loop (single GPU, native PyTorch).

Verifies Qwen3AttentionStage trains correctly in isolation using MSE loss
...
from examples.attn_moe_overlap.model_utils import Qwen3AttentionStage, create_qwen3_config
...
def main():
    ...
    model = Qwen3AttentionStage(config).to(device=device, dtype=dtype)
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3)

    print(f"Attention stage parameters: {sum(p.numel() for p in model.parameters()):,}")
